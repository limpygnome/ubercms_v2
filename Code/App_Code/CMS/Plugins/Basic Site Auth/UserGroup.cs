...
namespace CMS
{
    namespace BasicSiteAuth
    {
        ...
        public class UserGroup
        {
            ...
            {
                Result r = conn.Query_Read("SELECT * FROM bsa_user_groups WHERE groupid='" + Utils.Escape(groupID.ToString()) + "';");
                if (r.Rows.Count == 1)
                    return load(r[0]);
                else
                    return null;
            ...
            public void save(Connector conn, bool forceInsert)
            {
                lock(this)
                {
                    ...
                    if (!forceInsert && loaded)
                        conn.Query_Execute(sql.compileUpdate("bsa_user_groups", "groupid='" + Utils.Escape(groupID.ToString()) + "'"));
                    else
                        groupID = int.Parse(conn.Query_Scalar(sql.compileInsert("bsa_user_groups", "groupid")).ToString());
                    modified = false;
                }
            }
            // Methods - Properties
