...
namespace CMS.BasicSiteAuth.Models
{
    ...
    public class AccountCode
    {
        ...
        /// <summary>
        /// Creates a new unpersisted account code.
        /// </summary>
        public AccountCode()
        {
        ...
        // Methods *****************************************************************************************************
        /// <summary>
        /// Generates and sets a new account code.
        /// </summary>
        ...
        /// <summary>
        /// Creates a new account code model and persists it.
        /// </summary>
        /// <param name="conn">Database connector.</param>
        /// <param name="type">Type of account code.</param>
        /// <param name="user">The user which owns the code.</param>
        /// <returns>A model or null.</returns>
        public static AccountCode create(Connector conn, CodeType type, User user)
        {
        ...
        /// <returns>A model or null.</returns>
        public static AccountCode create(Connector conn, CodeType type, int userID)
        {
        ...
        /// </summary>
        /// <param name="conn">Database connector.</param>
        /// <param name="code">Account code - 16 characters max.</param>
        /// <param name="type">Type of account code.</param>
        /// <param name="user">The user which owns the code.</param>
        /// <param name="datetimeCreated">The creation date and time, used for cleaning up/expiration.</param>
        /// <returns>A model or null.</returns>
        public static AccountCode create(Connector conn, string code, CodeType type, User user, DateTime datetimeCreated)
        {
        ...
        /// <param name="datetimeCreated">The creation date and time, used for cleaning up/expiration.</param>
        /// <returns>A model or null.</returns>
        public static AccountCode create(Connector conn, string code, CodeType type, int userID, DateTime datetimeCreated)
        {
            try
            {
                AccountCode c = new AccountCode();
                c.Code = code;
                c.Type = type;
                c.UserID = userID;
        ...
        /// <param name="conn">Database connector.</param>
        /// <returns>A model or null.</returns>
        public static AccountCode load(string code, CodeType type, Connector conn)
        {
            PreparedStatement ps = new PreparedStatement("SELECT * FROM bsa_account_codes WHERE code=?code;");
            ps["code"] = code;
            ps["type"] = (int)type;
        ...
        /// <param name="conn">Database connector.</param>
        /// <returns>A model or null.</returns>
        public static AccountCode load(string code, string email, CodeType type, Connector conn)
        {
            PreparedStatement ps = new PreparedStatement("SELECT * FROM bsa_account_codes AS rc LEFT OUTER JOIN bsa_users AS u ON u.userid=rc.userid WHERE rc.code=?code AND u.email=?email;");
            ps["code"] = code;
            ps["email"] = email;
            ps["type"] = (int)type;
            Result data = conn.queryRead(ps);
        ...
        /// <returns>A model or null.</returns>
        public static AccountCode load(ResultRow row)
        {
        ...
        public bool save(Connector conn)
        {
            ...
            if(persisted)
            {
                ...
                c.executeUpdate(conn, "bsa_account_codes");
            }
            else
            {
                int attempts = 0;
                while(attempts < 5)
                {
                    try
                    {
                        c.executeInsert(conn, "bsa_account_codes");
                        break;
                    }
        ...
        {
            conn.queryExecute("DELETE FROM bsa_account_codes WHERE userid='" + SQLUtils.escape(userID.ToString()) + "'");
        }
        /// <summary>
        ...
        public static void removeExpired(Connector conn)
        {
            ...
            conn.queryExecute("DELETE FROM bsa_account_codes WHERE datetime_created < '" + SQLUtils.escape(dt) + "' AND type='" + (int)CodeType.Recovery + "'");
        }
        /// <summary>
        ...
        {
            conn.queryExecute("DELETE FROM bsa_account_codes;");
        }
        /// <summary>
        ...
        {
            PreparedStatement ps = new PreparedStatement("DELETE FROM bsa_account_codes WHERE code=?code;");
            ps["code"] = code;
