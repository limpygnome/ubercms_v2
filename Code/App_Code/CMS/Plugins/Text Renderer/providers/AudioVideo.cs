...
namespace CMS.Plugins.TRProviders
{
    public class AudioVideo : CMS.Plugins.RenderProvider
    {
        ...
        public override void render(Data data, ref StringBuilder header, ref StringBuilder text, RenderType renderTypes)
        {
            ...
            {
                // YouTube
                foreach (Match m in Regex.Matches(text.ToString(), @"\[youtube\](?:http|https):\/\/(?:it.|www.)?youtube\.(?:com|.co.uk|.com.br|.fr|.jp|.nl|.pl|.es|.ie)\/watch\?(?:[A-Za-z0-9\&\=_\-]+)?(v=([A-Za-z0-9_\-]+))(?:&[A-Za-z0-9\&\=_\-]+)?\[\/youtube\]", RegexOptions.Multiline))
                ...
                // Vimeo
                foreach (Match m in Regex.Matches(text.ToString(), @"\[vimeo\](?:http|https):\/\/(?:www.)?vimeo.com/([0-9]+)\[\/vimeo\]"))
